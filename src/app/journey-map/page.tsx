...
export default function JourneyMapPDF() {
  ...
  return (
    <div className={`min-h-screen bg-blue-800 font-sans text-gray-800 transition-all duration-500 ${isFullscreen ? 'p-0 fixed inset-0 z-[100]' : 'relative py-12 px-4 sm:px-6 lg:px-8'}`}
         style={!isFullscreen ? {
      ...
      <div className={`relative z-10 mx-auto flex flex-col items-center ${isFullscreen ? 'w-full h-full' : 'max-w-7xl mt-8 mb-16'}`}>
        ...
        <div className={`relative z-10 bg-white shadow-2xl overflow-hidden flex flex-col transition-all duration-500 w-full ${isFullscreen ? 'h-full rounded-none' : 'rounded-3xl border-4 border-blue-900'} `} style={{ height: isFullscreen ? '100%' : '85vh' }}>
          ...
          <div className="w-full flex-grow relative bg-[#525659]">
             <object 
               data="/journey-map.pdf#toolbar=0&navpanes=0&scrollbar=0&statusbar=0&messages=0" 
               type="application/pdf"
               className="absolute inset-0 w-full h-full border-none"
               title="Journey Map PDF Document"
               style={{ backgroundColor: '#525659' }}
             >
               <p>Unable to display PDF file. <a href="/journey-map.pdf">Download</a> instead.</p>
             </object>
          </div>
